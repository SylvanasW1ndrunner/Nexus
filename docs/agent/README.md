...
本模块是 SchemaNaut 的通用 Agent 内核，当前首先服务 AI SQL，并为后续数据库治理、运维、分析和数据开发提供持续会话、项目上下文、计划与工作状态、自主工具使用、完成验证、Skills、MCP、子 Agent、文件产物、许可和可见轨迹。目标工具架构见[通用 Agent 工具系统调研与改造技术指导](06-general-agent-tooling-research-and-architecture-guide.md)。
...
| 通用 Agent 工具架构调研 | 对照 Codex、Claude Code 与 MCP，指导工具系统通用化改造  | [06-general-agent-tooling-research-and-architecture-guide.md](06-general-agent-tooling-research-and-architecture-guide.md) |
...
flowchart LR
    ...
    Explore --> Act["执行数据库 / 文件 / 进程 / Web / MCP / 子 Agent"]
    Act --> Project["结果最小投影"]
...
- Agent 可以按需读取/修改文件、运行有界进程、写 SQL 脚本、调用 Web/MCP 和委派子 Agent；宿主通过 Tool 允许列表决定本轮可用边界。
