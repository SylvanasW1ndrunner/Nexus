...
运行时最初只加载名称、描述、作用域和路径；正文、引用和脚本在 Skill 被显式或隐式选择后加载。CLI 支持 `/skills`、`/<skill-name> [args]`，并可提供 `$skill-name` 兼容输入。

Skills 指导工作流和工具组合，不能提高 `read / edit / full` 权限。硬安全和一致性规则继续由代码执行。
...
MCP 原始类型不能进入公共合同。适配器将 MCP 能力接入统一 Registry，同时保留来源与协议语义。未实现的 Sampling、Elicitation 等客户端能力不得虚假声明。
...
子 Agent 默认使用与主 Agent 相同的自适应循环和普通能力，继承项目、公共 Skill 目录、父 Session Skill 的独立副本、Tool Registry、MCP 和权限模式。子 Agent 激活或更新自身状态时不得写回父 Session 或其他 Session。

子 Agent 必须拥有独立上下文、任务状态、取消和用量记录。主 Agent 通过委派任务创建子 Agent，子 Agent 返回结论、证据和产物引用，不把完整子会话灌回主上下文。
