...
测试条件：1,000 个有效样本、并发 50、Node.js v24.14.0、Windows、i9-13900HX。测试使用进程内确定性 Provider，仅测 DBAgent 平台开销，不包含外部模型、网络和排队时间。
...
- 结论：DBAgent 调用与计量链路正常；当前模型对短请求仍产生大量推理 Token，长尾延迟主要取决于模型和供应商服务。3 个样本只作为当前环境基线，不代表供应商 SLA 或并发容量。
