...
- `ConnectionProfile.scope` 是发现数据的强制边界：Runtime 会把档案中的租户、组织、Project、环境和区域传播到每个资源；Connector 返回冲突范围时发现失败关闭。
...
- Driver 只执行携带授权上下文的动作；读取仍受超时、扫描量和结果规模限制。数据内容规则由用户的 Skills、数据库权限与宿主策略定义，不由 Agent 默认猜测。
...
- v0.1 的内置和注入式 Connector/Driver 默认与 SDK 或 Server 运行在同一 Node.js 进程，不提供 Connector 崩溃、原生依赖崩溃或 OOM 的进程级隔离。
- 第三方 JDBC Bridge、原生依赖或其他不受信 Connector 的进程/容器/服务隔离由宿主和部署负责；未建立该边界时不能宣称其故障不会影响核心服务。
...
| 不支持能力明确失败率          |              100% |
| 资源状态来源与时间覆盖率      |              100% |
| 10 万资源按 ID 查询 P95       |           ≤ 10 ms |
| 10 万资源单跳关系查询 P95     |           ≤ 50 ms |
| 动态能力解析 P95              |            ≤ 5 ms |
| 查询提交平台开销 P95          |           ≤ 20 ms |
| 取消传播平台开销 P95          |          ≤ 100 ms |
| 百万行 Mock 流式结果          | 内存增量 ≤ 64 MiB |
| 单项元数据变化触发全量重建    |                 0 |
...
- 读取默认允许，但副作用函数、锁、扫描成本和结果规模仍受运行时约束；数据内容是否可读由数据库权限、用户 Skills 与宿主策略决定。
- 不确定写入结果不得自动重试。
- Mock 不能替代真实厂商验收。
...
PostgreSQL 参考实现使用 `pg@8.x`（MIT）。直接实现 PostgreSQL Wire Protocol 会重复认证、TLS、连接池、取消和类型解码等成熟能力，且明显增加兼容风险，因此不采用。`pg` 是纯 JavaScript 运行时依赖，默认不要求原生扩展或运行时下载；数据库协议能力封装在 Driver/Connector 内，不进入公共资源合同。

当前安装包没有 JDBC Runtime、Python 环境、数据库原生客户端或其他平台二进制。未来接入必须依赖本地 Driver 的数据库时，应作为独立 Connector 评审其下载、缓存、许可证、进程隔离和离线失败行为，不能把它隐式塞进核心 SDK。
