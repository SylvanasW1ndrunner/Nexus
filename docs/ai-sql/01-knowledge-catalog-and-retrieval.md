...
SchemaNaut 执行并提交的 DDL 直接触发强一致刷新；事务回滚不更新。外部工具执行的 DDL 使用 stale-while-revalidate：已有 Catalog 立即可检索，Runtime 按新鲜度间隔在后台发现 Schema，不阻塞普通提问；精确资源读取或知识检索未命中时，强制等待一次去重后的刷新并重试。这样既优先使用知识库，也不会在每个问题前扫描整库。索引损坏时删除或隔离索引并回退到数据库访问，不能阻断数据库连接。
