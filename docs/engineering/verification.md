...
- 前十八次 GitHub Actions 发布复验（runs `34757899868`、`34761714006`、`34763736333`、`34764907067`、
  ...
  第十七次运行确认四类跨进程取消均通过；唯一失败是未知外部结果夹具仍沿用通用 1 秒 Tool deadline，Windows
  高 I/O 负载下 Handler 启动前 deadline 已过，Runtime 因而正确记录 `TOOL_TIMEOUT/not_applied`，而不是虚构
  `tool.unknown`。当前候选仅把两处未知结果语义夹具的 Tool deadline 设为 60 秒，并分别为风险重试授权和重启后
  禁止通用恢复绕过决策的完整用例声明 45 秒测试预算；Handler 仍在启动后抛出确认丢失错误，全部结果未知、
  精确授权、幂等重放、冲突拒绝与恢复边界断言保持不变，生产 Tool deadline、状态机和风险重试语义均未修改。
  第十八次运行确认 Core Agent 704 项与上述两条未知结果路径全部通过；唯一失败来自 Workspace Search 在 Windows
  短路径下遍历文件时，以原始操作路径构造已打开句柄的身份，而待比较身份已经由 `realpath` 规范为长路径，因而把
  同一文件误判为 `target_changed`。当前候选保留原始路径用于重开和重校验文件，仅让句柄身份及对外
  `WorkspaceSearchMatch.canonicalPath` 沿用已验证的规范路径；inode、类型、大小、时间、链接与工作区边界检查不变。
