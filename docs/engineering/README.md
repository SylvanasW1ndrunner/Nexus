# 工程文档

本目录存放 DBAgent 的开发实现文档。当前阶段统一使用中文；命令、路径、包名、IPC channel 和 API 名称保持英文。

## 核心入口

- [接口文档](./interfaces.md)
- [M2+ 非 UI 服务合约预留](./interfaces-m2-contracts.md)
- [模块开发说明](./modules.md)
- [M2+ 后端优先阶段模块边界](./m2-backend-boundaries.md)
- [分模块开发文档](./modules/README.md)
- [三 Agent 并行开发治理](./agent-governance.md)
- [官方插件化策略](./official-plugin-policy.md)
- [开源优先开发规范](./open-source-first.md)
- [测试策略](./test-strategy.md)
- [打包策略](./packaging.md)
- [发布流程](./release-process.md)

## 模板与记录

- [开发切片模板](./templates/development-slice.md)
- [用户级验收报告模板](./templates/acceptance-report.md)
- [开发切片记录](./slices/README.md)
- [架构决策记录](./adr/README.md)

## 版本与验收

- [M0-M1.5 阶段验收记录](./m0-m15-acceptance.md)
- [M0-M1.5 候选验收审计](./m0-m15-final-audit.md)
- [betaV0.1 测试版记录](./releases/betaV0.1.md)
- [BetaV0.1.1 优化记录](./releases/BetaV0.1.1.md)
