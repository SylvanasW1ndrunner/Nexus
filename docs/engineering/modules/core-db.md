...
该模块不直接访问 renderer，也不把数据库凭据暴露给前端。它的职责是把产品文档中的“先 cancel，超时后断开当前连接”规则做成可测试合同。`QueryRequest` 已支持调用方传入 `queryId`，`PostgresDriver.execute()` 会在结果中保留该 id；主进程 query workflow 会在执行前注册查询、执行后标记 completed/failed/cancelled，并通过 driver observer 写入 PostgreSQL `backendPid`。backend pid 优先读取 `pg` client 的 `processID`，如果驱动环境没有稳定暴露该字段，则用 `select pg_backend_pid()` 兜底获取。

`db:cancel-query` 收到取消请求后，如果 backend pid 已知，会调用 `PostgresDriver.cancel()` 执行 `pg_cancel_backend($1)`。PostgreSQL driver 为每个连接维护独立的取消池，`max=1`，只用于执行 backend cancel；业务查询池即使被 `pg_sleep` 或复杂查询占满，取消请求也不会排队等待业务连接释放。如果 pid 缺失或取消超时，则降级断开当前连接。
...
SQL 编辑器执行入口由 `resolveSqlEditorExecutionTarget()` 收口。调用方传入文档全文、选择区、光标、连接能力和只读状态后，模块返回：

- `sourceSql`：用户实际选择或光标所在的源 SQL。
- `executableSql`：真正交给 driver 的 SQL；EXPLAIN 模式会包装为 `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` 或非 analyze EXPLAIN。
- `queryRequest`：可直接传给现有 `IDatabaseDriver.execute()` 的请求。
- `executionPlan`：对源 SQL 的安全、确认、事务、回滚和 EXPLAIN 建议。
...
- `apps/desktop/src/main/query-workflow.postgres.integration.test.ts`：真实 PostgreSQL 查询取消端到端，覆盖 `pg_sleep`、backend pid 捕获、`pg_cancel_backend`、`cancelled` 历史状态、取消后连接复用，以及 `maxClients=1` 时独立取消池不被业务池阻塞。
