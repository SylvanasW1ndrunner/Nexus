...
import { LlmModelRegistry, type RegisterModelInput, type RegisteredLlmModel } from './model-registry.js';
...
export class LlmGateway {
  ...
  registerProvider(provider: LlmProvider, models: Array<Omit<RegisterModelInput, 'providerId'>> = []): void {
  ...
  registerModel(input: RegisterModelInput): RegisteredLlmModel {
    ...
    const providerModels = this.registry.listModels().filter((candidate) => candidate.providerId === model.providerId);
    const maxConcurrency = minimumDefined(providerModels.map((candidate) => candidate.limits.maxConcurrency));
    const requestsPerMinute = minimumDefined(providerModels.map((candidate) => candidate.limits.requestsPerMinute));
    const tokensPerMinute = minimumDefined(providerModels.map((candidate) => candidate.limits.tokensPerMinute));
  ...
  async execute(input: LlmGatewayChatInput): Promise<LlmGatewayResult> {
    ...
    await this.emit(execution.event('request.started', { estimatedTokens: execution.estimatedInputTokens }));
    await this.emit(execution.event('route.decided', {
    ...
    const firstRequest = withSelectedModel(input.request, execution.route.selected.model.model);
    const cached = input.cache?.enabled
      ? this.cache.get(input.context.tenantId, firstRequest, input.cache.namespace)
      : undefined;
    if (cached) {
    ...
    try {
      ...
      if (reservation) await this.emit(execution.event('budget.reserved', { estimatedTokens: reservation.estimatedTokens }));
    } catch (error) {
      const normalized = normalizeProviderError(error);
      await this.emit(execution.event('request.failed', {
        latencyMs: performance.now() - startedAt,
    ...
    try {
      for (let candidateIndex = 0; candidateIndex < execution.candidates.length; candidateIndex += 1) {
        ...
        for (let retry = 0; retry <= execution.maxRetries; retry += 1) {
          ...
          try {
            ...
            if (input.cache?.enabled) {
              this.cache.set(input.context.tenantId, firstRequest, resultResponse, {
            ...
            const opened = shouldAffectCircuit(lastError) && this.reliability.recordFailure(candidate.providerId);
            if (opened) {
              await this.emit(execution.event('provider.circuit_opened', {
                providerId: candidate.providerId,
                modelId: candidate.id,
              }));
            }
      ...
      const cost = sumAttemptCost(attemptUsages);
      if (reservation) {
        if (attemptUsages.length > 0) this.budget.commitActual(reservation.id, usage.totalTokens, cost);
        else this.budget.release(reservation.id);
      }
      if (attemptUsages.length > 0) await this.recordAttemptUsage(input.round, attemptUsages);
      await this.emit(execution.event(isAborted(normalized) ? 'request.cancelled' : 'request.failed', {
        latencyMs: performance.now() - startedAt,
        errorCode: normalized.code,
        promptTokens: usage.promptTokens,
        completionTokens: usage.completionTokens,
  ...
  async *stream(input: LlmGatewayChatInput): AsyncIterable<LlmChatStreamEvent> {
    ...
    try {
      for (let candidateIndex = 0; candidateIndex < execution.candidates.length; candidateIndex += 1) {
        const candidate = execution.candidates[candidateIndex] as RegisteredLlmModel;
        const provider = this.requireProvider(candidate.providerId);
        if (candidateIndex > 0) await this.emit(execution.event('provider.fallback', { providerId: candidate.providerId, modelId: candidate.id }));
        for (let retry = 0; retry <= execution.maxRetries; retry += 1) {
          attempts += 1;
          await this.emit(execution.event('provider.attempt', {
            providerId: candidate.providerId,
          ...
          try {
            ...
            for await (const event of iterable) {
              ...
              const contentEvent = event.type === 'text-delta' || event.type === 'tool-call-delta' || event.type === 'tool-call';
            ...
            await this.emit(execution.event('request.completed', {
              ...
              cost,
              attributes: { attempts, streaming: true, usageEstimated: totalUsage.estimated ?? false },
            }));
            return;
          } catch (error) {
            lastError = deadline.timedOut() ? timeoutError(input.timeoutMs) : normalizeProviderError(error);
            ...
            const opened = shouldAffectCircuit(lastError) && this.reliability.recordFailure(candidate.providerId);
            if (opened) await this.emit(execution.event('provider.circuit_opened', { providerId: candidate.providerId, modelId: candidate.id }));
            if (!isRetryable(lastError) || retry === execution.maxRetries) break;
            await this.emit(execution.event('provider.retry', {
              providerId: candidate.providerId,
              modelId: candidate.id,
  ...
  submitBatch(inputs: LlmGatewayChatInput[], options: { concurrency?: number } = {}): LlmAsyncJob<LlmGatewayResult> {
    return this.jobs.submit(inputs, options);
  }

  getJob(id: string): LlmAsyncJob<LlmGatewayResult> | undefined {
    return this.jobs.get(id);
  }

  cancelJob(id: string): LlmAsyncJob<LlmGatewayResult> | undefined {
    return this.jobs.cancel(id);
  }

  listJobs(): Array<LlmAsyncJob<LlmGatewayResult>> {
  ...
  async embed(input: {
    ...
    if (!provider.embed) throw new LlmProviderError('LLM_CAPABILITY_UNSUPPORTED', `Provider does not implement embeddings: ${model.providerId}`, false);
    const response = await this.reliability.execute(
      model.providerId,
      async () => provider.embed?.({ ...input.request, model: model.model }) as Promise<LlmEmbeddingResponse>,
      input.request.signal,
      input.request.input.reduce((total, value) => total + Math.max(1, Math.ceil(value.length / 3)), 0),
    );
  ...
  async rerank(input: {
    ...
    const provider = this.requireProvider(model.providerId);
    if (!provider.rerank) throw new LlmProviderError('LLM_CAPABILITY_UNSUPPORTED', `Provider does not implement rerank: ${model.providerId}`, false);
    const response = await this.reliability.execute(
      model.providerId,
      async () => provider.rerank?.({ ...input.request, model: model.model }) as Promise<LlmRerankResponse>,
      input.request.signal,
      Math.max(1, Math.ceil((input.request.query.length + input.request.documents.join('').length) / 3)),
    );
    this.reliability.recordSuccess(model.providerId);
    return response;
  }
  ...
  private prepareExecution(input: LlmGatewayChatInput, streaming: boolean) {
    ...
    if (input.providerId && input.request.model) this.ensureModel(input.providerId, input.request.model);
    ...
    if (input.request.responseFormat && input.request.responseFormat.type !== 'text') capabilities.add('structuredOutput');
    ...
    const task: LlmTaskProfile = {
      ...
      requirements: {
        ...
        minOutputTokens: Math.max(input.task?.requirements?.minOutputTokens ?? 0, requestedOutputTokens),
    ...
    const candidates = [route.selected.model, ...route.fallbacks.slice(0, maxFallbacks).map((item) => item.model)];
    ...
    return {
      ...
      maxCorrections: boundedInteger(input.maxStructuredCorrections ?? 1, 0, 2, 'maxStructuredCorrections'),
      event: (type: LlmTelemetryEvent['type'], fields: Partial<LlmTelemetryEvent> = {}): LlmTelemetryEvent => ({
        ...baseEvent,
        ...fields,
        type,
        timestamp: this.now().toISOString(),
  ...
  private reserveBudget(input: LlmGatewayChatInput, execution: ReturnType<LlmGateway['prepareExecution']>): LlmBudgetReservation | undefined {
  ...
  private async callWithStructuredCorrection(
    ...
    for (let correction = 0; correction <= maxCorrections; correction += 1) {
      ...
      try {
        ...
        if (!(error instanceof LlmProviderError) || error.code !== 'LLM_STRUCTURED_OUTPUT_INVALID' || correction === maxCorrections) throw error;
        await this.emit(execution.event('provider.retry', {
    ...
    throw new LlmProviderError('LLM_STRUCTURED_OUTPUT_INVALID', 'Structured output correction was exhausted.', false);
  }
  ...
  private validateResponse(
    ...
    if (validateToolCalls && request.tools) this.validator.validateToolCalls(response.toolCalls, request.tools);
  ...
  private ensureModel(providerId: string, model: string): RegisteredLlmModel {
    return this.registry.find(providerId, model) ?? this.registry.registerModel({ providerId, model });
  }

  private requireProvider(providerId: string): LlmProvider {
    const provider = this.registry.provider(providerId);
    if (!provider) throw new LlmProviderError('LLM_NO_ROUTE', `LLM provider is not registered: ${providerId}`, false);
    return provider;
  }

  private async recordAttemptUsage(round: RoundContext | undefined, attempts: AttemptUsage[]): Promise<void> {
    if (!this.usageTracker) return;
    if (round) {
      await this.usageTracker.recordLlmCall(round, sumUsage(attempts.map((attempt) => attempt.usage)));
      return;
    }
    for (const attempt of attempts) {
      await this.usageTracker.recordTokens(attempt.model.mode === 'managed' ? 'managed' : 'byok', attempt.usage);
    }
  }
  ...
  private routeCapability(
    ...
    return this.router.route({
      task: {
        ...
        requirements: {
          ...input.task?.requirements,
          capabilities: [...new Set([...(input.task?.requirements?.capabilities ?? []), capability])],
          ...(requiredModelIds === undefined ? {} : { requiredModelIds }),
        },
      },
      ...(input.policies === undefined ? {} : { policies: input.policies }),
...
function validateGatewayInput(input: LlmGatewayChatInput): void {
  if (!input.context.tenantId.trim()) throw new LlmProviderError('LLM_POLICY_VIOLATION', 'tenantId is required.', false);
  if (!input.context.taskType.trim()) throw new LlmProviderError('LLM_POLICY_VIOLATION', 'taskType is required.', false);
  if (input.request.messages.length === 0) throw new LlmProviderError('LLM_BAD_RESPONSE', 'At least one LLM message is required.', false);
  if ((input.providerId && !input.request.model) || (!input.providerId && input.request.model && !input.modelId)) {
    throw new LlmProviderError('LLM_POLICY_VIOLATION', 'providerId and request.model must be supplied together for a fixed route.', false);
  }
}
...
function estimateResponseUsage(request: LlmChatRequest, response: LlmChatResponse): LlmUsage {
  ...
  const completionTokens = Math.max(1, Math.ceil((response.text.length + JSON.stringify(response.toolCalls).length) / 3));
...
function sumAttemptCost(attempts: AttemptUsage[]): number {
  return attempts.reduce(
    (total, attempt) => total + (estimateModelCost(attempt.model, attempt.usage.promptTokens, attempt.usage.completionTokens) ?? 0),
    0,
  );
}
...
function normalizeProviderError(error: unknown): LlmProviderError {
  ...
  return new LlmProviderError('LLM_PROVIDER_ERROR', error instanceof Error ? error.message : String(error), true);
}
...
function timeoutError(timeoutMs: number | undefined): LlmProviderError {
  return new LlmProviderError('LLM_TIMEOUT', `LLM request timed out after ${timeoutMs ?? 0}ms.`, true);
}
...
async function cancellableDelay(ms: number, signal?: AbortSignal): Promise<void> {
  if (signal?.aborted) throw new LlmProviderError('LLM_ABORTED', 'LLM request was aborted by the user.', false);
