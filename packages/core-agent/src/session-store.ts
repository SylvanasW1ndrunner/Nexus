...
export class AgentSessionStore implements AgentSessionWriter, AgentSubagentStore, AgentRunStore {
  ...
  async save(input: SaveAgentSessionInput): Promise<AgentSessionSummary> {
    ...
    const session = redactPersistedAgentValue(
      sanitizePersistedAgentSession(sanitizePersistedSqlToolResults(input.session)),
    ) as AgentSession;
...
function parseSession(payload: string, normalizedMessages: AgentMessage[] = []): AgentSession {
  ...
  return redactPersistedAgentValue(
    sanitizePersistedAgentSession(sanitizePersistedSqlToolResults({
      ...currentSession,
      messages,
    })),
  ) as AgentSession;
...
function sanitizePersistedAgentSession(session: AgentSession): AgentSession {
  const sanitized = structuredClone(session) as AgentSession & Record<string, unknown>;
  delete sanitized.activeTools;
  delete sanitized.knowledgeSnapshot;
  if (Array.isArray(session.activeSkills)) {
    sanitized.activeSkills = session.activeSkills.slice(0, 100).map(({ name, scope }) => ({
      name,
      scope,
    }));
  }
  if (Array.isArray(session.capabilityStates)) {
    sanitized.capabilityStates = session.capabilityStates.slice(0, 100).map((state) => ({
...
function sessionMetadataPayload(session: AgentSession): AgentSession {
  return {
    ...
    ...(session.modelBinding === undefined
      ? {}
      : { modelBinding: structuredClone(session.modelBinding) }),
    ...(session.capabilityStates === undefined
      ? {}
      : { capabilityStates: structuredClone(session.capabilityStates) }),
...
function markdownSession(session: AgentSession): string {
  const lines = [
    ...
    ...(session.modelBinding
      ? [`- Model binding: ${session.modelBinding.connectionId}/${session.modelBinding.modelId}`]
      : []),
    '',
  ];
