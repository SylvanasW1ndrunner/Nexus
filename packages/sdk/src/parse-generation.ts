import { DatabaseAgentError } from './errors.js';
...
export function parseGeneratedSqlResponse(text: string): ParsedGeneratedSql {
  ...
  if (sqlFence?.[1]) {
    ...
    if (sql) {
      const explanation = trimmed.replace(sqlFence[0], '').trim();
      return {
        sql,
        explanation: explanation || '模型生成的只读查询。',
        assumptions: [],
      };
    }
  }
  ...
  if (/^(select|with|values)\b/i.test(trimmed)) {
    return {
      sql: normalizeSql(trimmed),
      explanation: '模型生成的只读查询。',
      assumptions: [],
    };
  }
...
function normalizeSql(value: string): string {
  const fenced = value.match(SQL_FENCE)?.[1];
...
function invalidResponse(message: string): DatabaseAgentError {
  return new DatabaseAgentError('LLM_RESPONSE_INVALID', message, true);
}
