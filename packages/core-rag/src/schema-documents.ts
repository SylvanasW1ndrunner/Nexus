...
export function buildSchemaDocuments(input: {
  ...
  for (const table of input.tables) {
    ...
    documents.push({
      ...
      tokens: tokenize([table.schema, table.name, table.comment, ...table.columns.map((column) => column.name)]),
      relationIds,
      metadata: {
        type: table.type,
...
export function tokenize(values: Array<string | undefined>): string[] {
  ...
  return unique([...asciiTokens, ...asciiTokens.flatMap(splitIdentifier), ...cjkTokens, ...cjkTokens.flatMap(cjkBigrams)]);
}
