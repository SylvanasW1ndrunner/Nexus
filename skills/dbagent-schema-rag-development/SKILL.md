...
3. Build progressive indexes:
   - Skeleton: schemas/tables/comments/FTS.
   - Hot tables: columns, constraints, FK, indexes, embeddings when available.
   - Long tail: background indexing.
   - On-demand: explicit user references.
4. Build relationship graph for FK, naming, usage, and semantic expansion.
5. Retrieve with hybrid strategy: exact match, FTS, vector when available, glossary, graph expansion, RRF/reranking.
...
- Include schemas with comments, composite keys, FK chains, views, functions, indexes, and similarly named tables.
- Test progressive indexing restart/recovery.
...
- Missing embedding provider must fall back to exact/FTS retrieval.
